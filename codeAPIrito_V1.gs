function mainFunction() {
  ...
  var api_key = "RGAPI-1ebf12f3-2c64-4032-96b5-f7ff741afbdb"; //reset toutes les 24H !!!!!!!
...
function getTimeline(playerID,playerIDOpp,matchId,api_key) {
  //matchId = "4429357001";
  //api_key = "RGAPI-184a3ff7-7d75-418f-b1b7-6337ecfdefa8"
...
function workTimelineData(data,playerID,playerIDOpp) {
  ...
  var data = totalGold - totalGoldOpp;
...
function workMatchData(pseudo,matchData,champions,matchId,api_key) {
  try {
    ...
    var diffGold10min = getTimeline(playerID,playerIDOpp,matchId,api_key);
    
    var data = [teamPlayer,winner,lane,championPick,championPickOpp,diffGold10min];
  } catch (e) {
    Logger.log(e);
  }
  return data;
...
function writeSheet(sheet,data,matchListData,matchData) {
  try {
    console.log([matchData[0],matchData[1],matchData[2],matchData[3],matchData[4],matchData[5]]);
    sheet.appendRow([data.name,data.accountId,matchListData,matchData[0],matchData[1],matchData[2],matchData[3],matchData[4],matchData[5]]);
  } catch(e) {
    Logger.log(e);
  }
}
